...
## Decision: Persist raw codes, mask them everywhere else
...
back from. A hash cannot be displayed. Keymaster persists PINs in its own store,
so the trust boundary is established practice for this stack. FR-025's
requirement is about logs and diagnostics, which is satisfied by a salted
`code_ref` that is stable within an installation and meaningless outside it.

**Alternatives considered**:

- Store only a hash and re-derive the code: impossible for collision-resolved
  and adopted codes, which are not derivable.
- Store codes encrypted with a key elsewhere in `.storage`: rejected, no real
  threat model improvement over the rest of `.storage` and it adds a key
...
**[NEEDS CLARIFICATION]** FR-004's "explicit operator recovery path" for orphaned
allocations has no home in this feature: #735 owns force re-issue and FR-024
forbids new configuration. The plan retains and reports orphans indefinitely.
Whether a diagnostics entry alone is enough, or whether a service should be
added here, is an owner decision.
