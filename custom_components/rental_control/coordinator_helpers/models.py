...
def _trim_name(name: str, max_length: int) -> str:
...
def _format_display_slot_name(
    ...
    if len(prefix) >= max_name_length:
        return _trim_name(display_name, max_name_length)
    return f"{prefix}{_trim_name(slot_name, max_name_length - len(prefix))}"
