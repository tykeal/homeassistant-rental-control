...
def _datetime_to_date(value: datetime.datetime | datetime.date) -> datetime.date:
    ...
    if isinstance(value, datetime.datetime):
        local: datetime.datetime = dt_util.as_local(value)
        return local.date()
...
class RentalControlCalendar(
    ...
    def _date_only(self) -> bool:
        ...
        return bool(
            self.coordinator.config_entry.data.get(CONF_DATE_ONLY, DEFAULT_DATE_ONLY)
        )
    ...
    def event(self) -> CalendarEvent | None:
        ...
        if not self._date_only:
            return upcoming
        start, end = _collapse_span(upcoming.start, upcoming.end)
